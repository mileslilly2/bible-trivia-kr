...
def is_known_place(s: Optional[str]) -> bool:
    return bool(s and s in KNOWN_PLACES)
...
def _guess_divine_entity(text_n: str) -> Optional[str]:
    if "angel of the LORD" in text_n or "angel of the Lord" in text_n:
...
# Genealogy
RE_BEGAT = re.compile(
    rf"\b(?P<parent>{NAME})\s+begat\s+(?P<children>{NAME_LIST})\b"
)

RE_BARE = re.compile(
    rf"\b(?P<parent>{NAME})\s+(?:also\s+)?(?:bare|bore)\s+(?P<children>{NAME_LIST})(?:\b|[,;.])"
)

# Rename
RE_CALLED_NAME = re.compile(
    rf"\bcalled\s+(?:his|her|their)(?:\s+[A-Za-z']+)?\s+name\s+(?P<name>{NAME_TOKEN})\b"
)

# Speech
RE_SAID_UNTO = re.compile(
    rf"\b(?P<speaker>{NAME})\s+(?:said|spake)\s+(?:unto|to)\s+(?P<listener>{NAME})\b"
)

RE_SAID_UNTO_TWO = re.compile(
    rf"\b(?P<speaker>{NAME})\s+(?:said|spake)\s+(?:unto|to)\s+(?P<listener1>{NAME})\s+and\s+(?:unto|to)\s+(?P<listener2>{NAME})\b"
)

# Violence
RE_SLEW = re.compile(
    rf"\b(?P<killer>{NAME})\s+(?:slew|killed)\s+(?P<victims>{NAME_LIST})\b"
)
...
RE_WENT_FROM_TO = re.compile(
    rf"\b(?P<traveler>{NAME})\s+went\s+from\s+(?P<source>{NAME})\s+to\s+(?P<destination>{NAME})\b"
)
...
RE_APPEARED_UNTO = re.compile(
    rf"\b(?P<entity>{NAME})\s+appeared\s+(?:unto|to)\s+(?P<recipient>{NAME})\b"
)
...
def extract_genealogy(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_BEGAT.finditer(text_n):
        ...
        if parent and children:
            for child in children:
                facts.append(_fact(
                    ...
                    child=child,
                    parent_gender="male",
                    source_pattern="begat",
                ))
        else:
            drops.append(_drop(ref, "kjv_parent_of_begat_invalid", text_n, extracted=m.groupdict()))

    for m in RE_BARE.finditer(text_n):
        parent = normalize_person_name(m.group("parent"))
        children = [normalize_person_name(x) for x in split_name_list(m.group("children"))]
        children = [x for x in children if x]
        if parent and children:
            for child in children:
                facts.append(_fact(
                    ...
                    source_pattern="bare_or_bore",
                ))
        else:
            drops.append(_drop(ref, "kjv_parent_of_bare_invalid", text_n, extracted=m.groupdict()))
...
def extract_rename(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_CALLED_NAME.finditer(text_n):
        ...
        if name:
            facts.append(_fact(
                ...
                ref,
                name=name,
                source_pattern="called_name",
            ))
        else:
            drops.append(_drop(ref, "kjv_rename_invalid", text_n, extracted=m.groupdict()))
...
def extract_speech(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_SAID_UNTO_TWO.finditer(text_n):
        ...
        if speaker and l1 and l2:
            facts.append(_fact("spoke_to", ref, speaker=speaker, listener=l1, source_pattern="said_or_spake_unto_two"))
            facts.append(_fact("spoke_to", ref, speaker=speaker, listener=l2, source_pattern="said_or_spake_unto_two"))
        else:
            drops.append(_drop(ref, "kjv_spoke_to_two_invalid", text_n, extracted=m.groupdict()))

    for m in RE_SAID_UNTO.finditer(text_n):
        speaker = normalize_person_name(m.group("speaker"))
        ...
        if speaker and listener:
            facts.append(_fact(
                ...
                speaker=speaker,
                listener=listener,
                source_pattern="said_or_spake_unto",
            ))
        else:
            drops.append(_drop(ref, "kjv_spoke_to_invalid", text_n, extracted=m.groupdict()))
...
def extract_killing(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_SLEW.finditer(text_n):
        ...
        if killer and victims:
            for victim in victims:
                facts.append(_fact(
                    ...
                    killer=killer,
                    victim=victim,
                    source_pattern="slew_or_killed",
                ))
        else:
            drops.append(_drop(ref, "kjv_killed_invalid", text_n, extracted=m.groupdict()))
...
def extract_travel(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_WENT_FROM_TO.finditer(text_n):
        ...
        if traveler and source and destination and is_known_place(source) and is_known_place(destination):
            facts.append(_fact(
                ...
                source_pattern="went_from_to",
            ))
        else:
            drops.append(_drop(ref, "kjv_traveled_from_to_invalid", text_n, extracted=m.groupdict()))
    ...
    for m in RE_WENT_TO.finditer(text_n):
        ...
        if traveler and destination and is_known_place(destination):
            ...
            drops.append(_drop(ref, "kjv_went_to_invalid", text_n, extracted=m.groupdict()))
    ...
    for m in RE_JOURNEYED_TO.finditer(text_n):
        ...
        if traveler and destination and is_known_place(destination):
            ...
            drops.append(_drop(ref, "kjv_journeyed_to_invalid", text_n, extracted=m.groupdict()))
...
def extract_roles(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for pattern, role_name, rule_name in [
        (RE_WAS_KING, "king", "kjv_role_king_invalid"),
        (RE_WAS_PROPHET, "prophet", "kjv_role_prophet_invalid"),
        (RE_WAS_PRIEST, "priest", "kjv_role_priest_invalid"),
    ]:
    ...
    for m in RE_BEGAN_TO_REIGN_OVER.finditer(text_n):
        ...
        if person:
            ...
            drops.append(_drop(ref, "kjv_role_began_to_reign_invalid", text_n, extracted=m.groupdict()))
    ...
    for m in RE_REIGNED_OVER.finditer(text_n):
        ...
        if person:
            ...
            drops.append(_drop(ref, "kjv_role_reigned_over_invalid", text_n, extracted=m.groupdict()))
...
def extract_appearance(ref: str, text: str) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
    ...
    for m in RE_APPEARED_UNTO.finditer(text_n):
        ...
        if entity and recipient:
            facts.append(_fact(
                ...
                entity=entity,
                recipient=recipient,
                source_pattern="appeared_unto",
            ))
        else:
            drops.append(_drop(ref, "kjv_appeared_to_invalid", text_n, extracted=m.groupdict()))
